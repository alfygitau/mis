...
import { sendOtp } from "../../sdk/auth/auth";
...
const SendOtp = () => {
  ...
  const sendMyOtp = async (e) => {
    setLoading(true);
    e.preventDefault();
    try {
      const response = await sendOtp(formatPhoneNumber(username));
  ...
  return (
    <div
      style={{
        backgroundImage: "url('/my_image.webp')",
      ...
      >
        <form onSubmit={sendMyOtp}>
          ...
          <p className="text-[14px] mb-[20px] text-center">
            Did you forget your password? Enter your mobile to allow for
            password reset. The system will send a one time password to your
            mobile phone.
          </p>
          <div className="flex flex-col gap-[5px] mb-[40px]">
            <label
